	...
	function submitData(){
		...
		if(title==null || title==''){
			...
			$.post("${pageContext.request.contextPath}/admin/blog/save.html",{'id':'${param.id}','title':title,'blogType.id':blogTypeId,
				...
				'content':content,'sumary':UE.getEditor('editor').getContentTxt().substr(0,155),'keyWord':keyWord},function(result){
				if(result.success){
					alert("博客修改成功！");
				}else{
					alert("博客修改失败！");
...
<div id="p" class="easyui-panel" title="修改博客" style="padding: 10px">
	<table cellspacing="20px">
		...
		<tr>
			<td>所属类别：</td>
			<td>
				<select class="easyui-combobox" style="width: 154px" id="blogTypeId" name="blogType.id"  panelHeight="auto">
